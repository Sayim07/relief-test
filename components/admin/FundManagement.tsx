...
export default function FundManagement() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Create Fund Form */}
      {showCreateForm && (
        <div className="bg-[#1a1a2e] border border-[#392e4e] rounded-lg p-6">
          <div className="flex justify-between items-center mb-4">
            <h3 className="text-lg font-semibold text-white">Create New Relief Fund</h3>
            <button
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-gray-400 mb-2">Fund Name *</label>
              <input
                type="text"
                value={newFund.name}
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={() => setShowCreateForm(false)}
                className="px-6 py-2 border border-[#392e4e] rounded-lg text-gray-300 hover:bg-[#0a0a1a]"
              >
                Cancel
              </button>
            </div>
        ...
        {funds.length === 0 ? (
          ...
          <div className="space-y-4">
            {funds.map((fund) => (
              <div
                key={fund.id}
                className="border border-[#392e4e] rounded-lg p-6 hover:bg-[#0a0a1a] transition-colors bg-[#0a0a1a]/50"
              >
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <h4 className="font-semibold text-lg text-white mb-1">{fund.name}</h4>
                    {fund.description && (
                      <p className="text-sm text-gray-400 mb-2">{fund.description}</p>
                    )}
                    <div className="flex items-center gap-4 text-sm">
                      <span className={`px-3 py-1 rounded-full text-xs font-medium ${fund.status === 'active' ? 'bg-green-900/30 text-green-400' :
                      ...
                      {fund.category && (
                        <span className="text-gray-400">Category: {fund.category}</span>
                      )}
                    </div>
                  </div>
                </div>
